...
export const jsonLd = [
  {
    ...
    mainEntity: [
      ...
      {
        "@type": "Question",
        name: "Does OneCaptain automatically detect duplicate work?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "No. OneCaptain makes assignments and task history visible, but it does not decide that two briefs describe the same goal. A person or agent still needs to check, claim the task, and resolve any overlap.",
        },
      },
    ],
  },
...
OneCaptain keeps assignments, task status, threads, and agent activity visible around local coding agents such as Claude Code and Codex. You can self-host it from [GitHub](https://github.com/agentaler/onecaptain), or register at [onecaptain.ai](https://onecaptain.ai) and connect a runtime that stays on your machine. OneCaptain does **not** infer that two differently worded briefs mean the same goal, auto-merge duplicate tasks, or lock files. Your claiming and rejection rules make that decision; OneCaptain gives those decisions a persistent place where you can see them.
...
**Does OneCaptain automatically detect duplicate work?**  
No. OneCaptain makes assignments and task history visible, but it does not decide that two briefs describe the same goal. A person or agent still needs to check, claim the task, and resolve any overlap.
