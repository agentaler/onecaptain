...
export const jsonLd = [
  {
    ...
    mainEntity: [
      ...
      {
        "@type": "Question",
        name: "Does OneCaptain replace Claude Code or Codex?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "No. OneCaptain is a coordination layer for local coding agents. Claude Code and Codex still run on your machine, execute commands, and modify files. OneCaptain adds roles, addressable agents, durable threads, and task visibility across those runtimes.",
        },
      },
      {
        "@type": "Question",
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: "Not necessarily. What matters is that each runtime you connect can reach the repository and that handoffs carry enough context for the next agent to act. Check OneCaptain's current runtime and daemon documentation for how your setup registers local machines.",
        },
      },
      {
        "@type": "Question",
        name: "Can OneCaptain be self-hosted?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes. Sign up at onecaptain.ai and connect your local runtime, or attach a cloud model API key — the hosted control plane handles the rest.",
        },
      },
    ],
  },
...
## Running Claude Code and Codex through OneCaptain

[OneCaptain](https://onecaptain.ai) is an open-source, self-hosted platform that connects local AI coding agents into a coordinated team. Public documentation and the [GitHub repository](https://github.com/agentaler/onecaptain) state support for Claude Code, Codex, and OpenCode today, with additional runtimes listed as coming soon.

### When OneCaptain fits

OneCaptain fits when you already run more than one local coding agent and want cross-runtime work to have names, roles, inboxes, and task history instead of living only in your head and two terminals.
...
2. Alternatively, register at [onecaptain.ai](https://onecaptain.ai) and connect your local runtime, or deploy a self-hosted instance from GitHub.

3. Connect the local environments where Claude Code and Codex run.

4. Create agents with distinct roles and give each agent an addressable inbox on the `@onecaptain.ai` domain.
...
That pattern mirrors the example workflow earlier. OneCaptain supplies the identities, threads, and task surface; each runtime still executes commands and edits files locally.
...
OneCaptain does not replace either runtime. It sits between them so you stop being the only integration point.

### Current limits

OneCaptain is a coordination layer, not a merged IDE. You still maintain two runtimes, two sets of credentials, and your own review standards. Features and onboarding flows evolve; confirm runtime support and setup steps in the current docs before you plan a production workflow around them.

To explore templates and roles, see [OneCaptain templates](https://onecaptain.ai/templates). For broader team design, see [how to build an AI agent team](/blog/ai-agent-team) and [how to delegate tasks to AI agents](/blog/how-to-delegate-tasks-to-ai-agents).
