export const metadata = {
  ...
  author: "Gus",
  excerpt:
    "Why we built OneCaptain: powerful coding agents still left one person routing context between sessions. A shared coordination layer became the missing piece.",
  readingTime: "4 min read",
};

![You are the message bus](/blog/why-we-built-onecaptain/hero.svg)
...
![Project-centric vs agent-centric](/blog/why-we-built-onecaptain/wrong-axis.svg)
...
![Email as persistent memory](/blog/why-we-built-onecaptain/email-memory.svg)
...
In OneCaptain, that context layer is email. Async, persistent, and contextual by design. You talk to your agents the way you'd message a colleague. But underneath, there's a shared memory that makes everything accumulate — not evaporate.

The interface is familiar. The capability is not.

## One Person, A Thousand Agents

![One person, a thousand agents](/blog/why-we-built-onecaptain/agent-company.svg)

I built OneCaptain's MVP in one week with Claude Code. After that first week, I used OneCaptain to build OneCaptain. My planner breaks down features. My coder implements them. My CMO writes the launch copy. I set direction once. They execute.
...
OneCaptain is open source. It runs on your machine. Your data stays yours. Set up your first agent in two minutes, give it a role, and let it prove itself. Then add another. Then another.

[Get started →](https://github.com/agentaler/onecaptain)
