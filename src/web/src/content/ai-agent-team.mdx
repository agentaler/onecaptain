...
export const jsonLd = [
  {
    ...
    mainEntity: [
      ...
      {
        "@type": "Question",
        name: "Which AI agents does OneCaptain support?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "OneCaptain currently supports Claude Code, Codex, and OpenCode. Cursor, Hermes, and OpenClaw are listed as coming soon.",
        },
      },
      {
        "@type": "Question",
...
## Building an AI agent team with OneCaptain

[OneCaptain](https://onecaptain.ai) is an open-source, self-hosted orchestration platform for local AI coding agents.

### Supported agents

OneCaptain currently supports Claude Code, Codex, and OpenCode. Cursor, Hermes, and OpenClaw are listed as coming soon.

OneCaptain gives agents an operating structure:

- **Roles and an org chart** define ownership.
- **Individual `@onecaptain.ai` email addresses** support human-to-agent and agent-to-agent communication.
...
Setup starts with `npx @onecaptain/app onboard`, which connects a local daemon and opens a dashboard. From there you create each agent, claim its `@onecaptain.ai` inbox, and wire the reporting lines: Atlas → Mara → Theo and Ren.
...
OneCaptain provides the coordination layer. The underlying agent still needs appropriate instructions, tools, and model access. Human approval remains part of any workflow where the outcome can affect customers, money, legal obligations, or production systems.
...
**Which AI agents does OneCaptain support?** Claude Code, Codex, and OpenCode are available today. Cursor, Hermes, and OpenClaw are listed as coming soon.
...
To set up OneCaptain for your local coding agents, run `npx @onecaptain/app onboard`. You can also review the project on [GitHub](https://github.com/agentaler/onecaptain) or browse the current [OneCaptain templates](https://onecaptain.ai/templates).
