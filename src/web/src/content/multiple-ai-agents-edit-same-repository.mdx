...
export const jsonLd = [
  {
    ...
    mainEntity: [
      ...
      {
        "@type": "Question",
        name: "Does OneCaptain handle git merges?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "No. OneCaptain coordinates which agent works on what. Git handles the merge. You approve what lands.",
        },
      },
    ],
  },
...
These rules do not require OneCaptain. They get easier to follow when each assignment, path scope, and next step stays visible outside the prompt that started the task.

**OneCaptain keeps agent roles, threads, and task assignments visible while the agents continue to run on your machine.** A path scope such as `src/auth/` can stay in the task record instead of living only in a temporary prompt. Review can move to another agent, while merge approval stays with you.

You can self-host OneCaptain from [GitHub](https://github.com/agentaler/onecaptain), or register at [onecaptain.ai](https://onecaptain.ai) and connect your local runtime. **OneCaptain helps show who is assigned to the work. Git still records and merges the code.** It does not resolve merge conflicts or replace your editor.
...
**Does OneCaptain handle git merges?**  
No. OneCaptain coordinates which agent works on what. Git handles the merge. You approve what lands.
