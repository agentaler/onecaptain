...
We built [OneCaptain](https://onecaptain.ai) as that coordination layer for local coding agents. Agents keep their own runtimes and in-tool memory. OneCaptain adds roles, email-native threads, and task handoffs that outlast a single chat window. See [supported agents](/blog/ai-agent-team#supported-agents) for the current runtime list, and [how to build an AI agent team](/blog/ai-agent-team) for roles and handoffs.
