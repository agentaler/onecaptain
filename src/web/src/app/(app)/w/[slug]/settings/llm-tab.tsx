...
export function LlmTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setSaved(await listLlmProviders(workspaceId));
    } catch (err) {
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      await saveLlmProvider(workspaceId, kind, {
        apiKey: apiKey.trim(),
  ...
  const onRemove = async () => {
    try {
      await removeLlmProvider(workspaceId, kind);
      setVerify({ status: "idle" });
      await load();
  ...
  const onVerify = async () => {
    ...
    try {
      const result = await verifyLlmProvider(workspaceId, kind, model.trim());
      setVerify(
        result.ok
          ? {
              status: "ok",
    ...
    <div className="space-y-8 max-w-xl" data-testid="llm-settings">
      ...
      {verify.status === "failed" && (
        ...
        >
          ...
          <p className="text-muted-foreground break-words">{verify.error}</p>
        </div>
      )}
    </div>
  );
