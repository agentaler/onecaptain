...
const samplePosts: BlogPost[] = [
  {
    ...
    agentSummary: "Read when you want the origin story of OneCaptain coordination.",
  },
  {
    slug: "ai-agent-team",
...
describe("buildLlmsTxt", () => {
  it("includes required sections and product facts", () => {
    ...
    expect(text.startsWith("# OneCaptain\n")).toBe(true);
    ...
    expect(text).toContain("## How OneCaptain fits");
    expect(text).toContain("## Contact");
    expect(text).toContain("Claude Code, Codex, and OpenCode");
    expect(text).toContain("https://github.com/agentaler/onecaptain");
    expect(text).toContain("https://onecaptain.ai/llms.txt");
  });
  ...
  it("lists posts with absolute blog URLs, author, and date", () => {
    ...
    expect(text).toContain(
      "[How to Build an AI Agent Team](https://onecaptain.ai/blog/ai-agent-team)"
    );
    expect(text).toContain("OneCaptain Team, published June 8, 2026");
  });
  ...
  it("prefers agentSummary over excerpt", () => {
    ...
    expect(text).toContain(
      "Read when you want the origin story of OneCaptain coordination."
    );
  ...
  it("allows a custom site URL", () => {
    ...
    expect(text).toContain(
      "[Why We Built OneCaptain](http://localhost:3000/blog/why-we-built-onecaptain)"
    );
