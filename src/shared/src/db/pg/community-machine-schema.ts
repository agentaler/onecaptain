...
// ../community-machine-schema.ts. Any schema change must be made in BOTH files until the
// Postgres cutover removes the originals (plans/postgres-port.md).
import { pgTable, text,
  json, index, uniqueIndex } from "drizzle-orm/pg-core";
...
// a new one.
export const communityMachineToken = pgTable(
...
export const communityMachine = pgTable(
  "community_machine",
  {
    id: text("id")
      .primaryKey()
    ...
    availableRuntimes: json("available_runtimes")
...
export const communityMachineCredential = pgTable(
...
export const communityBotBinding = pgTable(
...
export const communityAgentRunnerKey = pgTable(
