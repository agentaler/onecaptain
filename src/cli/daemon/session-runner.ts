...
const ATTACHMENTS_BASE = tempDir("onecaptain-attachments");
...
export async function runSession(input: SessionRunnerInput): Promise<void> {
  ...
  let seq = 0;
  let toolCount = 0;
  const BATCH_SIZE = Number(process.env.ONECAPTAIN_MESSAGE_BATCH_SIZE) || 20;
  const FLUSH_INTERVAL_MS = Number(process.env.ONECAPTAIN_MESSAGE_FLUSH_INTERVAL_MS) || 100;
