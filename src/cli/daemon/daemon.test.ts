...
vi.mock("./config.js", () => ({
  ...
  sessionRunnerLogDir: vi.fn(() => path.join("/tmp", "onecaptain", "daemon", "session-runners")),
  daemonLogFilePath: vi.fn(() => path.join("/tmp", "onecaptain", "daemon", "logs", "2026-01-01.log")),
  lastUpdateMarkerPath: vi.fn((profile?: string) =>
    profile ? path.join("/tmp", "onecaptain", `last_update_${profile}`) : path.join("/tmp", "onecaptain", "last_update"),
  ),
}));
...
describe("spawnSessionRunner", () => {
  ...
  it("opens a log file fd with taskId.log and passes it as stdio [ignore, fd, fd]", () => {
    ...
    expect(mockMkdirSync).toHaveBeenCalledWith(
      path.join("/tmp", "onecaptain", "daemon", "session-runners"),
      { recursive: true },
    );
    expect(mockOpenSync).toHaveBeenCalledWith(
      path.join("/tmp", "onecaptain", "daemon", "session-runners", "t1.log"),
      "a",
    );
  ...
  it("sets logFilePath in the serialized input", () => {
    ...
    expect(decoded.logFilePath).toBe(path.join("/tmp", "onecaptain", "daemon", "session-runners", "t1.log"));
  });
  ...
  it("deletes oldest files when over limit, keeping newest 500", () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(mockUnlinkSync).toHaveBeenCalledWith(
        path.join("/tmp", "onecaptain", "daemon", "session-runners", `${i}.log`),
      );
    }
  });
...
describe("daemon restart spawn", () => {
  ...
  it("restart spawn uses process.execPath with log file fd for stdio", async () => {
    ...
    expect(mockOpenSync).toHaveBeenCalledWith(
      path.join("/tmp", "onecaptain", "daemon", "logs", "2026-01-01.log"),
      "a",
      0o600,
    );
...
describe("daemon kill_task handling", () => {
  beforeEach(() => {
    ...
    process.env.ONECAPTAIN_KILL_TASK_MAX_WAIT_MS = "500";
    process.env.ONECAPTAIN_KILL_TASK_POLL_MS = "50";
    ...
    process.env.ONECAPTAIN_KILL_GRACE_MS = "10";
    process.env.ONECAPTAIN_KILL_VERIFY_MS = "100";
  });

  afterEach(() => {
    for (const t of intervalTimers) realClearInterval(t);
    delete process.env.ONECAPTAIN_KILL_TASK_MAX_WAIT_MS;
    delete process.env.ONECAPTAIN_KILL_TASK_POLL_MS;
    delete process.env.ONECAPTAIN_KILL_GRACE_MS;
    delete process.env.ONECAPTAIN_KILL_VERIFY_MS;
  });
  ...
  it("reports 'target not found' after retry timeout when PID never appears", async () => {
    ...
    await startDaemon();
    // Wait longer than ONECAPTAIN_KILL_TASK_MAX_WAIT_MS (500ms) + overhead
    await new Promise((r) => setTimeout(r, 800));
