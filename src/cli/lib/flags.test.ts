...
describe("resolveAgentId", () => {
  const originalEnv = process.env.ONECAPTAIN_AGENT_ID;
  ...
  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env.ONECAPTAIN_AGENT_ID = originalEnv;
    } else {
      delete process.env.ONECAPTAIN_AGENT_ID;
    }
  });
  ...
  it("returns flag value when both flag and env are set (flag wins)", () => {
    process.env.ONECAPTAIN_AGENT_ID = "env_agent";
  ...
  it("calls process.exit(1) when neither is provided", () => {
    ...
    expect(errSpy).toHaveBeenCalledWith(
      "Error: --agent_id is required (or set ONECAPTAIN_AGENT_ID env var)",
    );
