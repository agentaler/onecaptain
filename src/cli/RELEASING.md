...
3. Build the Node ESM bundle (inlines `@onecaptain/shared` and its deps)
4. Publish `@onecaptain/cli@X.Y.Z` to npm via **OIDC + provenance**
...
Use tags like `cli/v0.2.0-beta.1` or `cli/v1.0.0-rc.1`. The npm `latest` dist-tag is **not** automatically moved for prereleases — users pin with `npm i @onecaptain/cli@0.2.0-beta.1`.
...
```bash
npm unpublish @onecaptain/cli@0.2.0
```

After 72h, use deprecation instead:

```bash
npm deprecate @onecaptain/cli@0.2.0 "broken release, please upgrade to 0.2.1"
```
