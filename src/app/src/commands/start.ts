...
export function startCommand(): Command {
  return new Command("start")
    .description("Start OneCaptain services")
    ...
    .action(async (opts) => {
      if (!isInstalled()) {
        console.error("Error: OneCaptain not installed. Run 'npx @onecaptain/app onboard' first.");
        process.exit(1);
      }
      ...
      startServices(ports, { foreground: !!process.env.ONECAPTAIN_PROJECT_ROOT });
